...
class DotsApp:
	...
	def _drop_complete(self):
		...
		def f(x):
			if isinstance(x, list):
				for data in x:
					yield filter(lambda x: isinstance(x, int), data)

		if all(list(d) for d in f(self._objectives.get_status())):
			self.reset()
	...
	def _menu(self, master):
		...
		fileMenu.add_cascade(label='New Game', menu=self.reset, underline=0)
